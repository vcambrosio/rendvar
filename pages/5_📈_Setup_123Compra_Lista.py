        ...
        if candle_atual["High"] >= entrada_target:
            ...
            if eden_trades:
                ...
                if eden_trades:
                    mme_curta_atual = candle_atual.get(f"MME_{mme_curta}", 0)
                    mme_longa_atual = candle_atual.get(f"MME_{mme_longa}", 0)
                    mme_curta_ant = df.iloc[setup['index']][f"MME_{mme_curta}"]
                    mme_longa_ant = df.iloc[setup['index']][f"MME_{mme_longa}"]

                    cond_eden = (
                        preco_entrada > mme_curta_atual and
                        preco_entrada > mme_longa_atual and
                        mme_curta_atual > mme_longa_atual and
                        mme_curta_atual > mme_curta_ant and
                        mme_longa_atual > mme_longa_ant
                    )
    ...
    if df_filtrado_resultados.empty:
        ...
        if resultado_detalhado is not None:
            ...
            if isinstance(df_trades_raw, pd.DataFrame) and len(df_trades_raw) > 0:
                # Se o filtro Éden está ativo, mostrar apenas trades que passaram nele
                if eden_trades and "Éden Ativo" in df_trades_raw.columns:
                ...
                if df_trades.empty:
                    ...
                    st.markdown(f"### 📊 Distribuição dos Retornos - {ativo_selecionado_detalhes}")
                    fig_hist = go.Figure()
                    ...
                    # Define cores: verde para lucros, vermelho para prejuízos
                    cores_mensal = ["mediumseagreen" if val >= 0 else "indianred" for val in retorno_mensal["Lucro"]]
                    ...
                    fig_bar = go.Figure(data=[
                        go.Bar(
                            ...
                            marker_color=cores_mensal,  # aplica cores condicionais
