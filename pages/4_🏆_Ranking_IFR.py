...
ifr_max = st.sidebar.slider("IFR Máximo", min_value=5, max_value=30, value=10, step=1)
...
usar_stop = st.sidebar.checkbox("Usar stop loss", value=False)
...
if st.button("🚀 Calcular Ranking"):
    ...
    for idx, ativo in enumerate(ativos_escolhidos):
        ...
        data_inicial_disponivel = pd.to_datetime(df_atv["Date"]).min()

        # 🚨 Verificação: ignorar ativos com menos de 10 anos de histórico
        if (data_final - data_inicial_disponivel).days < 365 * 10:
            ativos_excluidos.append({"Ativo": ativo, "Motivo": "Menos de 10 anos de histórico"})
            progress_bar.progress((idx + 1) / total)
            continue
