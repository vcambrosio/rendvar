...
st.set_page_config(page_title="Backtest Max/Min Múltiplo", layout="wide")

# Caminho dos dados
parquet_path = "01-dados/ativos_historicos.parquet"

st.title("📈 Backtest - Setup Máximas/Mínimas - Múltiplos Ativos")
...
candles_minima_min = st.sidebar.number_input("Mínima dos últimos X candles (mínimo)", min_value=1, max_value=30, value=5)
candles_minima_max = st.sidebar.number_input("Mínima dos últimos X candles (máximo)", min_value=candles_minima_min, max_value=30, value=20)
candles_minima_range = range(candles_minima_min, candles_minima_max + 1)
...
def backtest_ativo(ativo, df_filtrado, data_inicial, data_final, 
                  candles_minima_range, candles_maxima, usar_keltner, periodo_keltner,
                  desvio_keltner, usar_timeout, timeout_candles,
                  usar_stop, stop_pct, capital_inicial):
    
    resultados = []
    
    for candles_minima in candles_minima_range:
        ...
        for i in range(candles_minima, len(df)):
            ...
            if posicao:
                ...
                elif usar_timeout and dias_hold >= timeout_candles:
                    sair = True
...
if st.button("▶️ Executar Backtest Múltiplo"):
    ...
    for idx, ativo in enumerate(ativos_escolhidos):
        ...
        melhor_resultado = backtest_ativo(
            ...
            candles_minima_range, candles_maxima, usar_keltner, 
            periodo_keltner if usar_keltner else 0, desvio_keltner if usar_keltner else 0,
            usar_timeout, timeout_candles if usar_timeout else None,
            usar_stop, stop_pct if usar_stop else None,
            capital_inicial
        )
        
        if melhor_resultado["Candles_Minima"] is not None:
    ...
    df_melhores = pd.DataFrame([{
        "Ativo": res["Ativo"],
        "Candles_Minima": res["Candles_Minima"],
        "Trades": res["Trades"],
    ...
    if df_filtrado_resultados.empty:
        ...
        if resultado_detalhado is not None:
            ...
            if isinstance(df_trades_detalhado, pd.DataFrame) and len(df_trades_detalhado) > 0:
                st.markdown(f"### Trades do ativo {ativo_selecionado_detalhes} - Candles Mínima: {resultado_detalhado['Candles_Minima']}")
